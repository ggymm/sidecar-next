...
use crate::plugins::system::basic_info::{BasicInfo, collect_basic_info};
...
impl HomePage {
    ...
    fn build_cpu_card(&self, info: &BasicInfo) -> impl IntoElement {
        div()
            ...
            .child(self.build_info_row("逻辑核心".to_string(), format!("{} 个", info.cpu.logical_cores)))
    ...
    fn build_memory_card(&self, info: &BasicInfo) -> impl IntoElement {
        div()
            ...
            .child(self.build_info_row("使用率".to_string(), format!("{:.1}%",
                (info.memory.used as f64 / info.memory.total as f64) * 100.0)))
    }
    ...
    fn build_os_card(&self, info: &BasicInfo) -> impl IntoElement {
        div()
            ...
            .child(self.build_info_row("Shell".to_string(), info.env.shell.clone()))
        ...
        div()
            ...
            .child(
                if let Some(network) = &info.network {
                    ...
                    div()
                        .child(self.build_info_row("状态".to_string(), "网络信息获取失败".to_string()))
                }
            )
    }
}
